...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AdminContext.Provider value={{ role }}>
      <main>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-8 flex flex-col md:flex-row gap-4 md:gap-10">
          <aside className="relative md:w-56 shrink-0">
            ...
            <nav className="relative flex md:flex-col gap-1 overflow-x-auto no-scrollbar -mx-1 px-1 md:mx-0 md:px-0">
              {liens.map(({ href, label, Icon }) => {
                ...
                return (
                  <Link
                    key={href}
                    href={href}
                    className={`flex items-center gap-2 md:gap-2.5 px-3 md:px-3.5 py-2 md:py-2.5 rounded-xl text-[13px] md:text-sm font-medium whitespace-nowrap transition-colors ${
